...
for(let i=0; i< (phrases.length); i++){
    if (phrases[phraseId].length < phrases[i].length){
        phraseId=i;
    }
}
let longueur = phrases[phraseId].length;
let phrase = phrases[phraseId];

document.querySelector("#content").innerHTML = `<p>La phrase la plus longue est: "${phrase}" et elle fait ${longueur} caractères !</p>`
